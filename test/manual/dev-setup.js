...
(() => {
	...
	const editorQueryValue =
		parsedQuery[editorQueryName] || defaultEditorQueryValue;
	const pluginsQueryValue =
		parsedQuery[pluginsQueryName] || defaultPluginsQueryValue;
	...
	(() => {
		...
		div.innerHTML =
			...
			`<ul><li>${renderLoadingOptions(true)}${
	...
	global.loadEditor = (callback, PATH_PREFIX) => {
		...
		loadScript(`${PATH_PREFIX}${corePath}/loader.js`, () => {
			...
			if (isRelease) {
				...
				loaderPathsConfig[
					"vs/language"
				] = `${PATH_PREFIX}${pluginPath}/language`;
				loaderPathsConfig[
					"vs/basic-language"
				] = `${PATH_PREFIX}${pluginPath}/basic-language`;
				loaderPathsConfig["vs"] = `${PATH_PREFIX}${corePath}`;
			}
	...
	function renderLoadingOptions(isEditor) {
		...
		return `<strong style="width:130px;display:inline-block;">${name}</strong>:&#160;&#160;&#160;${Object.keys(
			paths,
		)
			.map((path) => {
				...
				return `<a href="${generateUrlForLoadingOption(
					isEditor,
					path,
				)}">${path}</a>`;
			})
